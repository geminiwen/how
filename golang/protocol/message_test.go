...
func TestTextRoundTripHTTPRequest(t *testing.T) {
	req := &HTTPRequestPayload{
		Method:  "POST",
		URL:     "/api/users?page=1",
		Headers: map[string][]string{"Content-Type": {"application/json"}},
	...
	env, err := NewTextHTTPRequest("req-text-1", req)
	if err != nil {
		t.Fatal(err)
	}

	data, err := MarshalText(env)
	if err != nil {
		t.Fatal(err)
	}
	...
	if decoded.RequestID != "req-text-1" {
		t.Fatalf("expected request_id 'req-text-1', got %q", decoded.RequestID)
	}

	payload, err := DecodeTextPayload[HTTPRequestPayload](decoded)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTextRoundTripHTTPResponse(t *testing.T) {
	resp := &HTTPResponsePayload{
		StatusCode: 200,
		Headers:    map[string][]string{"Content-Type": {"text/plain"}},
		Body:       []byte("hello"),
	...
	env, err := NewTextHTTPResponse("req-text-2", resp)
	if err != nil {
		t.Fatal(err)
	}
	...
	payload, err := DecodeTextPayload[HTTPResponsePayload](decoded)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTextRoundTripError(t *testing.T) {
	env, err := NewTextError("req-text-3", ErrClientNotFound, "client not found")
	if err != nil {
		t.Fatal(err)
	}
	...
	payload, err := DecodeTextPayload[ErrorPayload](decoded)
	if err != nil {
		t.Fatal(err)
	}
